(function () {
  ...
  function renderTemporaryBePanel(nation, national) {
    ...
    return `
      <section class="editor-rail-panel temporary-be-panel ${isOpen ? "is-open" : ""}">
        <button class="temporary-be-toggle" type="button" data-action="toggle-temporary-be-drawer" aria-expanded="${isOpen ? "true" : "false"}">
          <span class="temporary-be-arrow" aria-hidden="true">${isOpen ? "&lsaquo;" : "&rsaquo;"}</span>
          <span class="temporary-be-toggle-copy">
            <span class="section-kicker">Budget Expenditure</span>
            <strong>${safeText(fmtNumber(effectiveBe))}</strong>
          </span>
        </button>
        <div class="temporary-be-drawer" ${isOpen ? "" : "hidden"}>
