(function () {
  ...
  function fmtExplainPointValue(numeric, suffix = "percentage points") {
    return `${Number(numeric.toFixed(4)).toLocaleString("en-US", { maximumFractionDigits: 4 })} ${suffix}`;
  }
  ...
  function formatExplainValue(value, format = "auto") {
    ...
    if (format === "points") return fmtExplainPointValue(numeric);
  ...
  function scheduleStatExplain(target) {
    clearStatExplainTimer();
    statExplainTimer = setTimeout(() => showStatExplain(target), 140);
  }
  ...
  function fieldLabel(dataset, path) {
    const labels = {
      ...
      mobilizationEffectiveness: "Mobilization Effectiveness",
      mobilizationAbility: "Mobilization Ability",
      mobilizationEnduranceYears: "Mobilization Endurance",
      ...
      governmentalEfficiencyRisk: "Governmental Efficiency Risk",
      ...
      temporaryBudgetExpenditureCount: "Temporary BE Count",
      debt: "Debt",
  ...
  const internalChangeKeys = new Set([
    ...
    "national.wartimeBudgetPeakBonus",
    "national.wartimeBudgetBonus",
    "national.wartimeBudgetAutoExpenditure",
    "national.wartimeBudgetHeadroom",
    "national.mobilizationFinanceLevel",
    "national.mobilizationYears",
    "national.mobilizationReadiness",
    "national.mobilizationStrain",
    "national.mobilizationEffectiveness",
    "national.mobilizationAbility",
    "national.mobilizationEnduranceYears",
    "national.temporaryBudgetExpenditureCount",
  ...
  function isInternalChangeKey(key = "") {
    return internalChangeKeys.has(key) || key.startsWith("national.temporaryBudgetExpenditures.");
  }

  function visibleChangeImpacts(entry) {
    return (entry?.changes || entry?.deltas || []).filter((change) => !isInternalChangeKey(change.key));
  }
  ...
  function snapshotChanges(before, after) {
    return Array.from(new Set([...Object.keys(before || {}), ...Object.keys(after || {})]))
      .filter((key) => !isInternalChangeKey(key))
  ...
  function recordDirectNationChange({ id, dataset, field, label, beforeValue, afterValue, beforeSnapshot }) {
    const entry = {
      key: `${dataset}:${field}:${id}:${Date.now()}`,
      nationId: id,
      nationName: byId(id)?.name || id,
      dataset,
      field,
      label: label || fieldLabel(dataset, field),
      beforeValue,
      afterValue,
      changedAt: new Date().toISOString(),
      changes: snapshotChanges(beforeSnapshot, nationSnapshot(data, id)),
      deltas: []
    };
    entry.deltas = entry.changes.filter((change) => change.numeric && change.delta !== 0);
    data.meta.changeHistory = [entry, ...(data.meta.changeHistory || [])].slice(0, 60);
    return entry;
  }

  function addTemporaryBeFromEditor() {
    const id = state.selectedNation;
    const nation = byId(id);
    const national = data.national?.[id];
    if (!nation || !national) {
      state.notice = "Select a country first.";
      render();
      return;
    }
    const label = String(document.getElementById("temporaryBeLabel")?.value || "Temporary BE").trim() || "Temporary BE";
    const amount = Math.max(0, Math.round(Engine.number(document.getElementById("temporaryBeAmount")?.value, 0)));
    const years = Math.max(0, Math.ceil(Engine.number(document.getElementById("temporaryBeYears")?.value, 0)));
    if (!amount || !years) {
      state.notice = "Temporary BE needs an amount and at least 1 year.";
      state.editorTemporaryBeOpen = true;
      render();
      return;
    }
    const beforeSnapshot = nationSnapshot(data, id);
    const beforeTotal = fmtNumber(Engine.temporaryBudgetExpenditureTotal(national));
    const item = Engine.addTemporaryBudgetExpenditure(national, {
      label,
      amount,
      yearsRemaining: years,
      createdYear: data.meta.currentYear
    });
    if (!item) {
      state.notice = "Temporary BE could not be added.";
      state.editorTemporaryBeOpen = true;
      render();
      return;
    }
    Engine.recalculateAll(data);
    const afterTotal = fmtNumber(Engine.temporaryBudgetExpenditureTotal(national));
    recordDirectNationChange({
      id,
      dataset: "national",
      field: "temporaryBudgetExpenditure",
      label: "Added Temporary BE",
      beforeValue: beforeTotal,
      afterValue: `${afterTotal} (${label}, +${fmtNumber(amount)} for ${fmtNumber(years)} yr${years === 1 ? "" : "s"})`,
      beforeSnapshot
    });
    state.editorTemporaryBeOpen = true;
    saveWorkingState(`${nation.name} temporary BE +${fmtNumber(amount)} for ${fmtNumber(years)} year${years === 1 ? "" : "s"}.`);
  }

  function removeTemporaryBeFromEditor(itemId = "") {
    const id = state.selectedNation;
    const nation = byId(id);
    const national = data.national?.[id];
    if (!nation || !national) {
    ...
    const beforeSnapshot = nationSnapshot(data, id);
    const beforeTotal = fmtNumber(Engine.temporaryBudgetExpenditureTotal(national));
    const removed = Engine.removeTemporaryBudgetExpenditure(national, itemId);
    if (!removed) {
      state.notice = "Temporary BE item was not found.";
      state.editorTemporaryBeOpen = true;
      render();
      return;
    }
    ...
    recordDirectNationChange({
      ...
      dataset: "national",
      field: "temporaryBudgetExpenditure",
      label: "Removed Temporary BE",
      beforeValue: beforeTotal,
      afterValue: `${afterTotal} (removed ${removed.label})`,
      beforeSnapshot
    });
    state.editorTemporaryBeOpen = true;
    saveWorkingState(`${nation.name} temporary BE removed: ${removed.label}.`);
  }
  ...
  app.addEventListener("click", async (event) => {
    ...
    if (actionButton) {
      ...
      if (recordsViews.handleAction?.(action, actionButton)) {
        ...
        restoreArchivedNationFromEditor();
      } else if (action === "toggle-temporary-be-drawer") {
        state.editorTemporaryBeOpen = !state.editorTemporaryBeOpen;
        render({ force: true });
      } else if (action === "add-temporary-be") {
        addTemporaryBeFromEditor();
      } else if (action === "remove-temporary-be") {
        removeTemporaryBeFromEditor(actionButton.dataset.beId || "");
