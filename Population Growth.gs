...
function updatePopulation(currentYear) {
  ...
  const stabilityValues = names.map(n => [nationalStatusSheet.getRange(nationalRowMap[n], stabilityColumn).getValue()]);
  const unrestValues = names.map(n => [nationalStatusSheet.getRange(nationalRowMap[n], unrestColumn).getValue()]);
  const developmentValues = names.map(n => [nationalStatusSheet.getRange(nationalRowMap[n], developmentColumn).getValue()]);
  const economicHealthValues = names.map(n => [nationalStatusSheet.getRange(nationalRowMap[n], economicHealthColumn).getValue()]);
  const immigrationRates = names.map(n => [nationalStatusSheet.getRange(nationalRowMap[n], immigrationRateColumn).getValue()]);
