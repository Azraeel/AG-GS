...
function updateTradeStats() {
  ...
  const updatedTradeStats = names.map((nationName) => {
    ...
    const budgetCapacity = parseFloat(nationalStatusSheet.getRange(nationalRow, budgetCapacityColumn).getValue());
    const corruption = parseFloat(nationalStatusSheet.getRange(nationalRow, corruptionColumn).getValue()) / 100;
    const population = parseFloat(nationalStatusSheet.getRange(nationalRow, populationColumn).getValue());
    const factories = parseFloat(industrialSheet.getRange(industrialRow, civilianFactoriesColumn).getValue());
    const shipyardCount = parseFloat(industrialSheet.getRange(industrialRow, shipyardsColumn).getValue());
    const development = parseFloat(nationalStatusSheet.getRange(nationalRow, developmentColumn).getValue());
    const importReliance = parseFloat(tradeSheet.getRange(tradeRow, importRelianceColumn).getValue());
    const exportReliance = parseFloat(tradeSheet.getRange(tradeRow, exportRelianceColumn).getValue());
    const tradeDiversity = parseFloat(tradeSheet.getRange(tradeRow, economicTradeDiversityColumn).getValue());
    const nationalEconomicHealth = nationalStatusSheet.getRange(nationalRow, economicHealthColumn).getValue();
  ...
  // Update the sheet
  tradeSheet.getRange(5, tradeCapacityColumn, names.length, 6).setValues(updatedTradeStats);
}