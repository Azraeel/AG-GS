...
function onEdit(e) {
  ...
  if (e.range.getA1Notation() === yearCell.getA1Notation()) {
    ...
    const updatedIndustrial = names.map((nation) => {
      ...
      const eRow = economicRowMap[nation];
      ...
      const healthStatus = economicSheet.getRange(eRow, healthColumn).getValue();
      const tradeBalance = parseFloat(tradeSheet.getRange(tRow, tradeBalanceColumn).getValue()) || 0;
      const mobilizationLevel = militarySheet.getRange(mRow, mobilizationColumn).getValue() || "None";
...
function calculateBudget() {
  ...
  const currentBudgetCapacities = names.map(n => nationalSheet.getRange(nationalRowMap[n], budgetCapacityColumn).getValue());
  const currentBudgetBalances = names.map(n => nationalSheet.getRange(nationalRowMap[n], budgetBalanceColumn).getValue());
  ...
  const factoryValues = names.map(n => [industrialSheet.getRange(industrialRowMap[n], factoryColumn).getValue()]);
  const militaryFactoryValues = names.map(n => [industrialSheet.getRange(industrialRowMap[n], militaryFactoryColumn).getValue()]);
  const shipyardValues = names.map(n => [industrialSheet.getRange(industrialRowMap[n], shipyardColumn).getValue()]);
  const developmentLevels = names.map(n => [nationalSheet.getRange(nationalRowMap[n], developmentLevelColumn).getValue()]);
  const populationValues = names.map(n => [populationSheet.getRange(populationRowMap[n], populationColumn).getValue()]);
  const corruptionValues = names.map(n => [nationalSheet.getRange(nationalRowMap[n], corruptionColumn).getValue()]);
  const economicHealthStatuses = names.map(n => [nationalSheet.getRange(nationalRowMap[n], economicHealthColumn).getValue()]);
  const taxRates = names.map(n => [nationalSheet.getRange(nationalRowMap[n], taxRateColumn).getValue()]);
  ...
  const mobilizationLevels = names.map(n => militarySheet.getRange(militaryRowMap[n], mobilizationColumn).getValue());
